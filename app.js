...
app.post('/mainPost', function(req, res) {
	...
	switch (router) {
		...
		case 'editUser': {
			...
			//Записываем его в БД и ставим замыкание
			realmHelper.editUser(user.hashId, firstName, lastName, address, login, email, password, function(
				user,
				msg
			) {
				if (!user) {
...
function uploadFile(sampleFile, callback) {
	let id = uuidv1() + '.jpg';
	sampleFile.mv('./images/' + id, function(
		err // если функция мв вернет ошибку,то сработает замыкание
	) {
		if (err) {
